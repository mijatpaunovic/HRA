...
All the files required to reproduce the results of the study are included - code, input data, and intermediate results. Note that the provided HRV recordings are the result of the preprocessing of raw electrocardiogram (ECG) used in the study, and therefore do not require additional preparation. Running the scripts for specific subject groups requires minimal editing which is explained in description section at the beginning of scripts. The repository is structured as follows:

#### 1. Python code
- `nonlinear_measures.py` – Computes nonlinear measures across four short-term timescales (1-, 5-, 10-, and 20-minute HRV segments) for two subject groups.  
